...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        PlayerControl.Instance.AddBrick(listBrick, checkGridPlayer);

        if (listBrick.Count > 0)
        {
            PlayerControl.Instance.RemoveBrick(listUnBrick, checkGridPlayer);
        }
        if (PlayerControl.Instance.isFinish(finish, checkGridPlayer))
        {
            return;
        }
    ...
    {
        while (gameManager.transform.childCount != 0)
        {
            Destroy(gameManager.transform.GetChild(0).gameObject);
        }
    }

    private string[] ReadFile()
    {
        string curentMap = listLevel[0].name.ToString();
    ...
    private void MoveToTarget()
    {
        ...
        PlayerControl.Instance.Move(directPlayer);
